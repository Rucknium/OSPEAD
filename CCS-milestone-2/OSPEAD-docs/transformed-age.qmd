...
dsa.to.uniform <- function(x, block.height, crod, start_height.RingCT, output.index.only.locked, OBOE.prop) {
  ...
  G_star <- function(x) {
    decoyanalysis::wallet2_gamma_cdf(as.numeric(x), v, z,
      GAMMA_SHAPE, GAMMA_RATE, RECENT_SPEND_WINDOW)
    # Rust code
  }


  crod.reversed <- abs(diff(rev(c(0, crod.at.tx.construction))))[-(1:(CRYPTONOTE_DEFAULT_TX_SPENDABLE_AGE-1))]
  # Remove first 9 blocks before cumsum() since can't spend from those outputs
  ...
  if (TRUE) {
    ...
    y_1 <- crod.reversed[-1]
    dsa.pmf.correct <- (G_star(y_1 + 1) - G_star(y_0)) / (y_1 + 1 - y_0)

    dsa.pmf.correct <- rep(dsa.pmf.correct, times = diff(crod.reversed))
    dsa.pmf.correct[length(dsa.pmf.correct)] <- dsa.pmf.correct[length(dsa.pmf.correct) - 1]
    ...
    dsa.pmf.correct[unspendable.indices] <- 0
  ...
  dsa.pmf <- rep(c(0, dsa.pmf), times = c(first.post.unlock.block, diff(crod.reversed)))
  ...
  if (TRUE) {
    ...
    collapse::setop(dsa.pmf, "*", OBOE.prop)
    collapse::setop(dsa.pmf.correct, "*", 1 - OBOE.prop)
    collapse::setop(dsa.pmf, "+", dsa.pmf.correct)
    # rm(dsa.pmf.OBOE, dsa.pmf.correct)
  }

  # dsa.pmf <- dsa.pmf / sum(dsa.pmf)
  collapse::setop(dsa.pmf, "/", collapse::fsum(dsa.pmf))

  dsa.cdf <- cumsum(dsa.pmf[1:max(x)])
  # Only need to compute up to the maximum x index
  # Maybe there is a way to speed this up even more by not generating the
  # full cumsum vector
